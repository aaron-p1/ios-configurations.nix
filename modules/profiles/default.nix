...
let
  ...
  profileOptions = mapAttrs (_: toProfileOption) configs;
  defaultProfileConfig = mapAttrs (_: toDefaultProfileConfig) configs;
  profileAssertions = concatMap ({ value, ... }: toProfileAssertions value) (attrsToList configs);
  ...
  toDefaultProfileConfig =
    ...
    // {
      ...
      PayloadIdentifier = mkDefault config.meta.id;
      PayloadUUID = mkDefault config.meta.uuid;
    };

  toProfileAssertions =
    pConfig:
    let
      profileConfig = cfg.${pConfig.name};
    ...
    in
    if profileConfig.enable then
      (genAssertions pConfig.supportData profileConfig pConfig.name [ ]) ++ pConfigAssertions
    else
      [ ];

  genAssertions =
    supportData: configValue: name: prevKeys:
    concatMap (
      { name, value }:
      if name == "*" then
        throw "wc"
  ...
  genAssertionsSingleKey =
    ...
    let
      keys = (prevKeys ++ [ key ]);
      propPathString = concatStringsSep "." ([ name ] ++ keys);

      listAssertions =
        if supportData ? "*" then
          concatMap (elem: genAssertions supportData."*" elem name (keys ++ [ "*" ])) configValue
        else
          [ ];
    ...
    if supportData ? minIos then
      ...
      genAssertions supportData configValue name keys;

  profileConfigValues = pipe configs [
    (mapAttrsToList (name: _: cfg.${name}))
